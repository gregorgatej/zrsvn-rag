...
def handle_query(user_query, selected_model_name, num_context_items, use_lexical_search, use_reranking):
    ...
    if num_context_items > 0 and 'context_source' in formatted_response and 'context_content' in formatted_response:
        for i, (source, content) in enumerate(zip(formatted_response['context_source'], formatted_response['context_content']), start=1):
            context_info += f"Source {i}: {source}  \nText: {content}  \n\n"
...
def build_interface():
    with gr.Blocks() as demo:
        gr.Markdown("# Question Answering with RAG and LLMs")
        gr.Markdown("Enter your question below and receive an answer generated by the selected Language Model.")
        
        with gr.Column(elem_id="main_column", scale=1):
            question = gr.Textbox(label="Your Question", lines=2, placeholder="Enter your question here...")
            model_selector = gr.Dropdown(choices=list(llm_options.keys()), label="Choose a Language Model", value="OpenAI GPT-4o Mini")
            num_context_items = gr.Slider(minimum=0, maximum=10, step=1, value=5, label="Number of Context Items")
            use_lexical_search = gr.Checkbox(label="Use Lexical Search")
            use_reranking = gr.Checkbox(label="Use Reranking")
            submit_button = gr.Button("Get Answer")
                
            answer_output = gr.Textbox(label="Answer")
            context_output = gr.Textbox(label="Context")

            # Add feedback mechanism with thumbs up/down
            feedback_buttons = gr.Radio(choices=["👍", "👎"], label="Did you find the answer helpful?")
            comment_box = gr.Textbox(label="Leave a comment (optional)", placeholder="Add your comment here...")
